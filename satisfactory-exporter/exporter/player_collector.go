...
func (c *PlayerCollector) Collect(ch chan<- prometheus.Metric) {
	...
	for _, d := range details {
		ch <- prometheus.MustNewConstMetric(PlayerPosition, prometheus.GaugeValue, d.Location.X, d.PlayerName, fmt.Sprintf("%f", d.ID), "X")
		ch <- prometheus.MustNewConstMetric(PlayerPosition, prometheus.GaugeValue, d.Location.Y, d.PlayerName, fmt.Sprintf("%f", d.ID), "Y")
